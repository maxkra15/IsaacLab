...
def _apply_offline_kit_args(args_cli: argparse.Namespace) -> None:
    ...
    The waterhose assets ship with this task, so the only remaining external calls Kit makes are
    (1) the extension registry on Azure
    ...
    try:
        ...
        while _simulation_is_running(env) and step < args_cli.max_steps:
            if control_graph_captured:
                ...
                if env._sim_step_counter % env.cfg.sim.render_interval == 0 and env.sim.is_rendering:
                    env.sim.render(skip_app_pumping=not env.render_enabled)
                terminated = truncated = None
            elif scripted_state is not None:

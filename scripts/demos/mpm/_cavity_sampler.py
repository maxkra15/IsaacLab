...
"""Private teapot-demo sampler for hollow mesh cavities using Warp point-mesh queries.

The sampler fills the enclosed air cavity of a consistently oriented, closed, double-walled shell
such as the Utah teapot. A winding-number volume fill of such an asset selects only the thin wall
material, so each candidate is instead kept when it is outside the wall material and surrounded
along most axis-aligned rays. This heuristic is not a general enclosure test for open vessels or
arbitrary concave meshes.
"""
...
def _validate_lattice_settings(
    ...
    """Validate cavity-lattice sampling settings."""
...
@wp.kernel
def _mark_cavity_points_kernel(
...
def sample_particles_in_cavity(
    ...
    *,
    ...
    This fills the *empty cavity* of a consistently oriented asset such as the closed, thin,
    double-walled Utah teapot. A winding-number "interior" is the wall material rather than the
    cavity, so each lattice point is kept when it is outside the shell material and most of its six
    axis rays hit the surface. This criterion does not prove enclosure for open vessels or arbitrary
    concave meshes. Optionally, only fill up to a :paramref:`water_level`.

...
* configure :class:`~isaaclab_contrib.coupling.CoupledProxySolverCfg` so the crates are
...
def read_snow_reaction_force() -> float | None:
    ...
    wrenches = NewtonCoupledSolverManager.get_proxy_body_wrenches(RIGID_ENTRY, SNOW_ENTRY)
    if wrenches is None:
        return None

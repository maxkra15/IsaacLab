...
* Changed Franka Pour shaping to bounded signed open-approach, contact-grasp, lift, and alignment
  ...
  progress through a validated 140-degree side-grasp roll, added a held-only stage-zero reference for its
...
* Enlarged the Franka Pour source cup to a 56 x 56 x 119 mm glass while preserving its media volume
  and keeping the visible geometry, particle collider, and grasp proxy aligned. The Panda's
  seven-waypoint horizontal approach now keeps both the tool and jaw axes parallel to the table,
...
* Fixed the randomized curriculum exposing full receiver displacement and reset-arm jitter at its
  first level by building an equal-size, balanced IK census at every source, receiver, yaw, and TCP
  perturbation extent. Every frontier now has the same pose distribution, the final bank retains
  the full configured amplitudes, and replay uses the immediately preceding frontier.
* Fixed the randomized grasp waypoint missing the tall source glass's authored side-grasp point,
  and strengthened the bounded Panda preload so the media-loaded glass remains held through the
  validated carry and 140-degree pour trajectory.
* Fixed randomized Franka Pour IK reset banks depending on training-batch layout by solving and
  collision-validating every candidate in an environment-local coordinate frame.
* Fixed randomized reset sampling overrepresenting source locations with more surviving IK rows by
  assigning every eligible source cell equal probability, covering a broad collision-screened
  side-grasp sector on both sides of the nominal reach line, and requiring at least two safe arm
  starts per retained cell.
* Fixed the first randomized curriculum frontier changing receiver clearance and IK branches
  discontinuously by adding an exact full-task anchor, scaling every geometric perturbation from
  zero, centering low-discrepancy receiver samples on the authored pose, preferring the authored
  receiver branch, and growing the number of exposed arm-start variants with physical extent before
  progressively exposing the complete source, target, and arm-reset domain.
* Fixed receiver IK being selected before its source-relative carry branch by jointly ranking and
  collision-screening complete reset-to-pour paths across source, pour, and tilt IK alternatives.
* Fixed one infeasible initial IK row aborting the complete reset-bank build by sanitizing its
  continuation seed and filtering the row from sampleable reset pools under the existing workspace
  coverage guard.
* Fixed the final Franka Pour curriculum starting from a cup-aligned tool pose by adding
  curriculum-scaled, collision-screened approach-depth and reset-orientation variation while
  retaining aligned pre-grasp and grasp waypoints.
* Fixed high-level Franka Pour randomization remaining effectively nominal by expanding the source
  candidate region to a 40--78 cm, +/-35-degree reach sector with +/-30-degree cup yaw, expanding
  the receiver and collision-screened arm-reset domains, and using a 65-percent intermediate
  promotion threshold while retaining 80-percent success as the full-domain mastery criterion.

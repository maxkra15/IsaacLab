...
def test_task_has_receiving_cup_safe_actions_and_success_threshold():
    ...
    assert cfg.actions.arm_action.waypoint_count == 7
    assert cfg.actions.arm_action.residual_scale == pytest.approx((0.03,) * 7)
    assert cfg.actions.arm_action.waypoint_phases == pytest.approx((0.0, 0.10, 0.20, 0.30, 0.46, 0.66, 1.0))
    assert (
        cfg.actions.arm_action.approach_waypoint,
        cfg.actions.arm_action.grasp_waypoint,
        cfg.actions.arm_action.lift_waypoint,
        cfg.actions.arm_action.align_waypoint,
    ) == (1, 3, 4, 5)
    assert cfg.actions.arm_action.phase_rate == pytest.approx(0.40)
    assert cfg.actions.arm_action.approach_phase_rate == pytest.approx(0.30)
...
def test_training_curriculum_closes_by_default_and_penalizes_irrecoverable_spill():
    ...
    assert cfg.actions.gripper_action.scale == pytest.approx(0.003)
...
def test_backward_curriculum_config_is_complete_and_play_uses_randomized_task():
    ...
    assert cfg.curriculum_randomization_extent_levels == pytest.approx((0.0, 0.05, 0.15, 0.30, 0.50, 0.75, 1.0))
...
@pytest.mark.parametrize(
    ...
    [
        ("waypoint_count", 6),
        ("grasp_waypoint", 1),
        ("waypoint_phases", (0.0, 0.10, 0.20, 0.30, 0.46, 0.46, 1.0)),
    ],
)

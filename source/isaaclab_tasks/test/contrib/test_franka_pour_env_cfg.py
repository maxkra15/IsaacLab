...
def test_finalize_builds_independent_assets_and_propagates_overrides():
    ...
    cfg.target_cup_inner_width = 0.16
    ...
    assert isinstance(source.spawn, CubeBowlSpawnerCfg)
    ...
    assert isinstance(target.spawn, CubeBowlSpawnerCfg)
    assert target.spawn.inner_width == pytest.approx(0.16)
    ...
    assert media.spawn.mass == pytest.approx(cfg.media_particle_spacing**3 * cfg.media_material.density)
    assert media.spawn.radius == pytest.approx(0.5 * cfg.media_particle_spacing)
    assert len(media.spawn.positions) == _reset_dataset_task_contract(resolved)["particle_count"]
...
def test_play_mode_freezes_single_world_playback_without_changing_policy_abi():
    ...
    assert cfg.viewer.eye == (0.9, 0.65, 0.5)
    assert cfg.sim.default_visualizer_cfg.eye == cfg.viewer.eye

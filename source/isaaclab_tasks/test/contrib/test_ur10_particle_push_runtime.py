...
if _RUNTIME_AVAILABLE:
    ...
    from isaaclab_tasks.contrib.ur10_particle_push.ur10_particle_push_env_cfg import MPM_ENTRY
...
@pytest.mark.skipif(not _RUNTIME_AVAILABLE, reason=_RUNTIME_UNAVAILABLE_REASON)
def test_randomized_reset_preserves_variable_payload_and_unselected_world():
    """Smoke-test variable payloads, split-pile resets, and selective exact reset."""
    sim_utils.create_new_stage()
    env = None
    try:
        ...
        assert observations["critic"].shape == (2, cfg.state_space)
        torch.testing.assert_close(observations["heightmap"][:, 2], task._heightmap_goal_mask)

        active_count = task._particle_active_mask.sum(dim=1)
        minimum_active_count = math.prod(cfg.reset_pile_lattice_min_resolution)
        maximum_active_count = math.prod(cfg.reset_pile_lattice_max_resolution)
        assert torch.all(active_count >= minimum_active_count)
        assert torch.all(active_count <= maximum_active_count)
        assert torch.all(active_count < task._media.particles_per_object)
        ...
        zero_action = torch.zeros((task.num_envs, cfg.action_space), device=task.device)
        ...
        active_position_e = task._particle_position_e()[0]
        source_mask = task._particle_active_mask[0] & (active_position_e[:, 0] < cfg.bin_inner_x_bounds[0])
        ...
        assert active_position_e[focused_mask, 1].mean() * active_position_e[other_source_mask, 1].mean() < 0.0
        ...
        original_minimum_crop = task.cfg.reset_pile_lattice_min_resolution
        task.cfg.curriculum_level_override = final_level
        task.cfg.reset_pile_lattice_min_resolution = task.cfg.reset_pile_lattice_max_resolution
        try:
            task._reset_idx(torch.tensor([0], dtype=torch.long, device=task.device))
        finally:
            task.cfg.curriculum_level_override = original_level_override
            task.cfg.reset_pile_lattice_min_resolution = original_minimum_crop

        assert task._particle_active_mask[0].all()
        assert task.episode_length_buf[0] == 0
        torch.testing.assert_close(task._particle_active_mask[1], protected_active_mask)

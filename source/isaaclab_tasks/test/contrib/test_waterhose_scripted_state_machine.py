...
def test_carry_target_recenters_connector_slip_within_phase():
    ...
    # centred on the socket. The validated predecessor used this live translational correction.
...
def test_insert_does_not_advance_on_hard_timeout_when_connector_is_not_retained():
    """INSERT must not report progress merely because its hard timeout elapsed."""
    ...
    # The fake connector is far from the socket. Reaching the generic 2x phase timeout must not
    # advance to HOLD_INSERTED, otherwise the runner can eventually reach DONE without insertion.
    state.compute(env)

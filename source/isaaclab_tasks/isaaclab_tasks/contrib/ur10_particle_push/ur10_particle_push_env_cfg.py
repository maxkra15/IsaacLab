...
# 3.375x particle cost of three samples per axis. A broad, shallow 24 x 32 x 8 reserve lattice
# holds 6,144 slots: essentially the former particle cost, but with a realistic sweep footprint.
# Reset-time contiguous crops vary the true payload without changing PPC or sample density. The
# volume-equivalent radius keeps Newton's ``8 * radius**3`` volume consistent with each sample.
...
@configclass
class UR10ParticlePushEnvCfg(DirectRLEnvCfg):
    """Direct, relative-joint-control UR10 particle-pushing task."""
    ...
    # DirectRLEnv exposes proprioception under ``policy`` and privileged state under ``critic``.
    # The image-shaped ``heightmap`` group is declared by the task and encoded separately by PPO.
    # The policy observes the active delivery target so the staged terminal condition is Markov.
    ...
    # Relative commands are offsets from the measured joint position, so this scale also bounds
    # the position error available to the PD actuator. With 2,400 N m/rad stiffness, the arm
    # offsets can reach roughly the UR10's 87 N m effort limit instead of stalling in the pile.
    # Velocity and effort limits remain enforced by the articulation.
    relative_joint_position_action_scale: tuple[float, ...] = (
        0.035,
        0.035,
        0.035,
        0.052,
    ...
    # The fixed reserve preserves model capacity and topology. Every episode activates one contiguous,
    # centered XY and bottom-supported Z crop. The 2,592--6,144 active slots vary true particle
    # count and material volume without thinning samples or changing radius/density.
    reset_pile_lattice_min_resolution: tuple[int, int, int] = (18, 24, 6)
    reset_pile_lattice_max_resolution: tuple[int, int, int] = PILE_LATTICE_RESOLUTION
    # Pack the active source material into visibly different, volume-preserving shapes. Each
    ...
    # A small number of grains may leave the physical spill band while PPO explores, but reset
    # before finite runaway particles can expand the NanoVDB hierarchy indefinitely.
    ...
    # Minimal potential shaping makes the physical objective legible before terminal success is
    ...
    reward_paddle_reach_potential_per_second: float = 0.005
    ...
    penalty_spill_fraction_per_second: float = -0.20
    # An unrecoverable state must not retain a positive return after collecting the complete
    # bounded shaping potential. Successful delivery still adds the separate terminal bonus.
    penalty_failure: float = -2.0
    # Relative joint commands integrate into position targets; penalizing command magnitude teaches
    # the policy to stop after a useful motion instead of drifting into a collider. These
    # continuous-time coefficients match the effective per-step regularization used by the
    # previously working push task while remaining invariant to policy rate.
    penalty_action_magnitude_per_second: float = -0.10
    penalty_action_rate_per_second: float = -0.01

    # Active sparse-grid capacity follows occupied solver cells, not the 6,144 reserve samples.
    ...
    def _validate_reset_config(self) -> None:
        ...
        if any(
            ...
            for minimum, maximum in zip(
                self.reset_pile_lattice_min_resolution,
                self.reset_pile_lattice_max_resolution,
                strict=True,
            )
        ):
    ...
    def _validate_particle_reset_envelopes(self) -> None:
        ...
        active_count = math.prod(self.reset_pile_lattice_max_resolution)
        ...
        for level, (center_x, delivered_fraction, pile_count, lateral_offset, scale) in enumerate(
            ...
            delivered_count = math.floor(active_count * delivered_fraction)
            maximum_group_count = math.ceil((active_count - delivered_count) / pile_count)
        ...
        maximum_delivered_count = math.floor(active_count * max(self.curriculum_initial_bin_fraction))
        ...
        delivered_half_x, delivered_half_y = _packed_group_half_extent(
            ...
            self.reset_pile_lattice_max_resolution[2],
            bin_aspect_ratio,
            0.0,
            self.reset_particle_jitter,
        )
    ...
    def __post_init__(self) -> None:
        ...
        self.sim.physics = NewtonCfg(
            ...
            num_substeps=1,
            # Reset-time particle activation uses Newton's public model-change notification,
            # which rebuilds solver-owned arrays and therefore requires eager execution.

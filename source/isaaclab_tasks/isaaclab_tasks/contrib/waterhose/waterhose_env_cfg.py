...
# The imported RBY1 contains dedicated collision references for these shells, but does not enable
# them as colliders. Author them onto the instantiated stage instead of modifying the separately
# distributed asset. They remain part of the MJWarp robot entry; only the four finger bodies are
# mirrored into VBD for cable contact.
_ROBOT_FRIDGE_COLLISION_ROOT_NAMES = frozenset(
    {
        "left_arm_tool_flange_collision",
        "left_arm_wrist_pitch_collision",
        "left_gripper_base_collision",
        "left_gripper_camera_bracket_collision",
        "right_arm_tool_flange_collision",
        "right_arm_wrist_pitch_collision",
        "right_gripper_base_collision",
        "right_gripper_camera_bracket_collision",
    }
)
_ROBOT_FRIDGE_CONTACT_SOLREF = (0.005, 1.0)
_ROBOT_FRIDGE_COLLISION_PROPERTIES = NewtonSDFCollisionPropertiesCfg(
...
def _spawn_rby1_with_fabric_compatible_geometry(
    ...
    **kwargs,
    ...
    """Spawn RBY1 with renderable references and its robot/fridge collision shells enabled.
    ...
    The source asset also carries collision-only meshes for the palm, camera bracket, tool flange,
    and wrist without collision schemas. Enable those meshes after de-instancing so MJWarp can keep
    both grippers out of the fridge housing. This runtime authoring keeps the asset tarball immutable.
    """
    ...
    _enable_rby1_robot_fridge_colliders(prim)
    return prim


def _enable_rby1_robot_fridge_colliders(robot_prim) -> tuple[str, ...]:
    """Enable the existing RBY1 gripper-shell meshes used for robot/fridge contact."""
    from pxr import Sdf, Usd, UsdGeom, UsdPhysics, Vt  # noqa: PLC0415

    stage = robot_prim.GetStage()
    meshes_by_root: dict[str, list] = {name: [] for name in _ROBOT_FRIDGE_COLLISION_ROOT_NAMES}
    for descendant in Usd.PrimRange(robot_prim):
        if not descendant.IsA(UsdGeom.Mesh):
            continue
        ancestor = descendant.GetParent()
        while ancestor and ancestor != robot_prim:
            root_name = ancestor.GetName()
            if root_name in meshes_by_root:
                meshes_by_root[root_name].append(descendant)
                break
            ancestor = ancestor.GetParent()

    missing_roots = sorted(name for name, meshes in meshes_by_root.items() if not meshes)
    if missing_roots:
        raise RuntimeError(
            "RBY1 robot/fridge collision meshes are missing under "
            f"{robot_prim.GetPath()}: {', '.join(missing_roots)}. "
            "Re-extract the matching waterhose asset tarball."
        )

    enabled_paths: list[str] = []
    for root_name in sorted(meshes_by_root):
        for mesh_prim in meshes_by_root[root_name]:
            mesh_path = str(mesh_prim.GetPath())
            sim_schemas.define_collision_properties(
                mesh_path,
                _ROBOT_FRIDGE_COLLISION_PROPERTIES,
                stage=stage,
            )
            mesh_collision = UsdPhysics.MeshCollisionAPI.Apply(mesh_prim)
            mesh_collision.CreateApproximationAttr().Set("none")
            mesh_prim.CreateAttribute("mjc:solref", Sdf.ValueTypeNames.DoubleArray).Set(
                Vt.DoubleArray(_ROBOT_FRIDGE_CONTACT_SOLREF)
            )
            enabled_paths.append(mesh_path)
    return tuple(enabled_paths)
...
# The 10 mm gap is only the broad-phase detection shell; the 1 mm margin is the
# physical contact surface. The light connector is rotated and carried against
# the cable-tail load, and lower proxy friction lets it shift inside the fingers
# enough to make insertion alignment stochastic. Preserve the validated grip
# coefficient while the socket/cable shapes retain their realistic lower values.
_VBD_GRIPPER_PROXY_FRICTION = 20.0
_VBD_GRIPPER_PROXY_MARGIN = 0.001
_VBD_GRIPPER_PROXY_GAP = 0.01
_GRIPPER_PROXY_MASS_SCALE = 1.0
_PROXY_RIGID_CONTACT_MIN = 30_000
_PROXY_RIGID_CONTACTS_PER_ENV = 256
# SolverVBD allocates warm-start history from this model-wide capacity before
# the proxy-local pipeline is constructed. It therefore bounds both the small,
# filtered outer manifold and the dynamically sized proxy manifold. This is an
# allocation limit, not a contact-material parameter.
_DEFAULT_RIGID_CONTACT_MAX = 131_072
...
_RIGHT_GRIPPER_CLOSE_COMMAND = {
    # Close to a 14 mm pad gap on the ~14.5 mm connector flange and grip through the contact margin band
...
def _make_proxy_collision_pipeline(model):
    ...
    # non-finger path plus finger contact with the connector mesh and articulated cable rods. The
    # pads constrain both the connector flange and the hose body instead of relying on connector
    # friction alone.
    import numpy as np
    ...
    # Imported shapes retain Newton's standalone defaults (ke=2500, kd=100,
    # mu=1), including excessive damping for this VBD hose path. Normalize only
    # the task-owned connector corridor; unrelated scene shapes retain their
    # authored materials.
    ...
    for shape_id in finger_shapes:
        # Mirrored finger shapes retain the RBY1 asset's standalone Newton
        # defaults. Apply the task's VBD contact units, especially the lower
        # damping needed by a connector that rotates in the grip.
        material_ke[shape_id] = _VBD_CONTACT_STIFFNESS
        material_kd[shape_id] = _VBD_CONTACT_DAMPING
    ...
    world_count = max(1, int(getattr(model, "world_count", 1)))
    proxy_contact_max = _env_positive_int(
        "WATERHOSE_PROXY_RIGID_CONTACT_MAX",
        max(_PROXY_RIGID_CONTACT_MIN, _PROXY_RIGID_CONTACTS_PER_ENV * world_count),
    )
    history_capacity = int(getattr(model, "rigid_contact_max", 0) or 0)
    if history_capacity and proxy_contact_max > history_capacity:
        raise ValueError(
            "waterhose proxy contact capacity exceeds the VBD history capacity "
            f"({proxy_contact_max} > {history_capacity}); increase WATERHOSE_RIGID_CONTACT_MAX "
            "to at least WATERHOSE_PROXY_RIGID_CONTACT_MAX."
        )
    ...
    return CollisionPipeline(
        ...
        rigid_contact_max=proxy_contact_max,
        # Preserve the contact patch across millimetre-scale connector motion
        # inside the gripper and socket.
...
@configclass
class RewardsCfg:
    """No reward terms: this branch is a scripted and teleoperated demo."""
...
class WaterhoseEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self) -> None:
        ...
        coupling_iterations = _env_positive_int("WATERHOSE_COUPLING_ITERS", 1)
        rigid_contact_max = _env_positive_int("WATERHOSE_RIGID_CONTACT_MAX", _DEFAULT_RIGID_CONTACT_MAX)
        ...
        self.sim.physics = NewtonCfg(
            ...
            solver_cfg=CouplerProxyCfg(
                entries=[
                    ...
                    CouplerEntryCfg(
                        name="vbd",
                        # Contact/solver recipe matched to Newton's
                        # franka_cable_ik_pick_place reference grasp. Hard
                        ...
                        solver_cfg=VBDSolverCfg(
                            ...
                            # Start new/unmatched hard contacts near the validated grip response.
                            # With beta=1e2, penetration-driven AVBD ramping alone adds only a few
                            # N/m per solve and the default 1e2 seed lets the light connector slip
                            # laterally during insertion.
                            ...
                            # The finite penalty seed is the load-bearing value for the
                            # soft tail weld; cable joints use their separately authored
                            # stretch/bend values.
                ...
                proxies=[
                    CouplerProxyMappingCfg(
                        ...
                        # Resolve the cable against the gripper pose from this substep. The generic
                        # lagged default is safer for loosely coupled scenes, but lets this
                        # millimetre-scale connector move one substep before proxy feedback is applied.
                        mode="staggered",
                        mass_scale=_GRIPPER_PROXY_MASS_SCALE,
                        # Use Newton's standard one-pass, fixed-feedback proxy recipe. Repeated
                        # relaxation passes restart the solver state while retaining a nonlinear
                        # contact manifold, which destabilizes this millimetre-scale grasp.
            ...
            # This model-wide capacity is visible before SolverVBD construction,
            # so rigid-contact history is allocated before CUDA graph capture.
            # Develop's coupler routes the shared outer contacts separately
            # from the proxy-local VBD contacts. The proxy pipeline scales its
            # own contact buffer with world count; increase this bound together
            # with WATERHOSE_PROXY_RIGID_CONTACT_MAX for larger batches.
            collision_cfg=NewtonCollisionPipelineCfg(rigid_contact_max=rigid_contact_max),
...
@configclass
class WaterhoseNewtonIkActionsCfg(WaterhoseRightGripperActionsCfg):
...
@configclass
class WaterhoseNewtonBimanualIkActionsCfg(WaterhoseRightGripperActionsCfg):

...
from isaaclab.utils.math import combine_frame_transforms, normalize, subtract_frame_transforms
...
class WaterhoseMimicEnv(ManagerBasedRLMimicEnv):
    ...
    def _get_cable_head_pose(
        ...
        """Read the live cable-head/plug pose from Newton and express it in the robot/root frame."""
        ...
        body_indices = [int(registry_entry.segment_body_indices[env_id][0]) for env_id in env_indices]
        body_indices_tensor = torch.tensor(body_indices, device=self.device, dtype=torch.long)
        body_q = wp.to_torch(NewtonManager.get_state_0().body_q).to(device=self.device)
        cable_head_pose_w = body_q[body_indices_tensor]
        cable_head_pos_w = cable_head_pose_w[:, :3]
        cable_head_quat_w = normalize(cable_head_pose_w[:, 3:])
        cable_head_pos_b, cable_head_quat_b = subtract_frame_transforms(
            root_pos_w, root_quat_w, cable_head_pos_w, cable_head_quat_w
        )

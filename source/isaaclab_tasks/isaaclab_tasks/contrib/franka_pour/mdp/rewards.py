...
def finite_joint_velocity_l2(
    ...
    """Return a bounded joint-velocity penalty on a terminal instability step.

    Failure predicates terminate non-finite simulation states before the next observation, but the
    reward manager evaluates the same terminal state first. Sanitizing this auxiliary penalty keeps
    that transition consumable by PPO without hiding the failure from termination statistics.
    """
...
def terminal_failure(env: FrankaPourEnv) -> torch.Tensor:
    """Return one unit-integral pulse for any unsuccessful episode completion."""
    success = _success_terminal(env)
    failed = env.termination_manager.dones & ~success

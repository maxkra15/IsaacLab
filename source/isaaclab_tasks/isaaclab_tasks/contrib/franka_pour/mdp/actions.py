...
class TrajectoryJointPositionAction(ActionTerm):
    ...
    def _approach_ready(self) -> torch.Tensor:
        tcp_error = self._env.tcp_pos_e() - self._env.cup_grasp_point_e()
        cup_velocity = self._env.cup_velocity_w()
        return (
            (torch.linalg.vector_norm(tcp_error[:, :2], dim=-1) <= self._approach_max_lateral_distance)
...
class TrajectoryJointPositionActionCfg(ActionTermCfg):
    ...
    """Phase rate before the grasp waypoint, kept slower for contact-safe descent [1/s]."""
    ...
    """Consecutive centered, stationary steps required before the guarded descent."""
...
class CurriculumGripperPositionAction(ActionTerm):
    """Filtered symmetric finger-position residual around a safe preload target."""
    ...
    @property
    def action_offset(self) -> torch.Tensor:
        """Per-environment zero-action finger target [m]."""
    ...
    def set_action_offset(
        ...
        """Set selected environments' zero-action finger target [m]."""
        selected = slice(None) if env_ids is None else env_ids
    ...
    def process_actions(self, actions: torch.Tensor) -> None:
        self._raw_actions.copy_(actions)
        target = torch.clamp(
            self._action_offset + self._scale * torch.clamp(actions, -1.0, 1.0),
        ...
        if self._force_open_stage >= 0:
            ...
            tcp_error = self._env.tcp_pos_e() - self._env.cup_grasp_point_e()
            lateral_distance = torch.linalg.vector_norm(tcp_error[:, :2], dim=-1)
            vertical_distance = torch.abs(tcp_error[:, 2])
            cup_velocity = self._env.cup_velocity_w()
            capture_required = self._env.curriculum_stage >= self._force_open_stage
            capture_ready = (
                (arm_action.reference_phase >= self._force_open_phase)
                & (lateral_distance <= self._capture_max_lateral_distance)
                & (vertical_distance <= self._capture_max_vertical_distance)
...
class CurriculumGripperPositionActionCfg(ActionTermCfg):
    ...
    joint_names: list[str] = MISSING
    scale: float = 0.04
    """Per-finger position residual represented by one policy-action unit [m]."""
    alpha: float = 0.2
    """Exponential smoothing weight applied to the commanded finger target."""
    ...
    """Per-finger zero-action command [m]. ``None`` uses ``close_position``."""
    ...
    """Maximum lateral TCP-to-grasp distance for symmetric finger contact [m]."""
    capture_max_vertical_distance: float = 0.008
    """Maximum vertical TCP-to-grasp distance within the source side walls [m]."""

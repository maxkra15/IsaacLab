...
that leave the task workspace before they can expand the rebuildable sparse grid without bound.
"""
...
def excessive_spill(env: FrankaPourEnv) -> torch.Tensor:
    """Terminate when strictly more than the allowed media fraction is spilled."""
    return env.spilled_fraction() > float(env.cfg.max_spill_fraction)
...
def lost_lifted_grasp(
    ...
    """Terminate after a demonstrated lift loses the physical cup grasp continuously.

    A short dwell rejects isolated contact-deflection flicker from the coupled rigid solver while
    still terminating a genuinely dropped cup before it can fall appreciably.
    """
    ...
    lost = lifted_before & ~preloaded_grasp
    ...
    return lost & (env._lost_grasp_dwell_count >= dwell_steps)
...
def stable_pour_success(
    ...
    candidate = (held_target_fraction >= env.pour_target_frac) & held_pour & ~env.termination_manager.terminated

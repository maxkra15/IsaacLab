...
class PourResetDatasetCurriculum(ManagerTermBase):
    """Sample reset rows and learn from asynchronous episode outcomes.

    Each reset records only the row identifier and its Boolean local-progress outcome. Distributed
    workers queue these pairs during asynchronous resets and exchange them once at the RSL-RL
    rollout boundary through :meth:`synchronize_pending_outcomes`.
    """
    ...
    def __init__(self, cfg: CurriculumTermCfg, env: FrankaPourEnv):
        ...
        self._pending_outcome_batches: list[tuple[torch.Tensor, torch.Tensor]] = []
    ...
    def __call__(
        ...
        if not env.cfg.curriculum_freeze and self._distributed_world_size() == 1:
    ...
    def synchronize_pending_outcomes(self) -> int:
        """Gather queued row/outcome pairs at one distributed rollout boundary.

        Returns:
            Number of globally gathered completed episodes.
        """
        local_rows, local_progress = self._pending_outcomes()
        world_size = self._distributed_world_size()
        if world_size == 1:
            if local_rows.numel() > 0:
                self._apply_outcomes(local_rows, local_progress)
            self._pending_outcome_batches.clear()
            self._refresh_metrics_cache()
            return int(local_rows.numel())

        local_count = torch.tensor([local_rows.numel()], device=self._device, dtype=torch.long)
        ...
        packed = torch.zeros((max_count, 2), device=self._device, dtype=torch.long)
        count = int(local_count.item())
        if count > 0:
            packed[:count, 0] = local_rows
            packed[:count, 1] = local_progress
        gathered = [torch.empty_like(packed) for _ in range(world_size)]
        dist.all_gather(gathered, packed)

        # Interleave completion index and rank so every worker applies the same bounded-history order.
        valid = torch.arange(max_count, device=self._device)[:, None] < counts[None, :]
        outcomes = torch.stack(gathered, dim=1)[valid]
        self._apply_outcomes(outcomes[:, 0], outcomes[:, 1].bool())
        self._pending_outcome_batches.clear()
        self._refresh_metrics_cache()
        return int(counts.sum().item())

    def _pending_outcomes(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Return locally queued outcomes without removing them."""
        if not self._pending_outcome_batches:
            return (
                torch.empty(0, device=self._device, dtype=torch.long),
                torch.empty(0, device=self._device, dtype=torch.bool),
            )
        rows, progress = zip(*self._pending_outcome_batches, strict=True)
        return torch.cat(rows), torch.cat(progress)
    ...
    def _distributed_world_size() -> int:
        """Return the initialized distributed world size without creating a process group."""
        if not dist.is_available() or not dist.is_initialized():
            return 1
        return dist.get_world_size()

    def _refresh_metrics_cache(self) -> None:
        """Refresh host logging metrics at a reset or rollout boundary."""
        self._metrics_cache = self._sampler.metrics()
        self._local_reset_assignments_since_metrics = 0

...
class ActionsCfg:
    ...
    arm_action = mdp.TrajectoryJointPositionActionCfg(
        ...
        alpha=0.10,
        phase_rate=0.40,
        # Give the horizontal insertion enough time to settle before the fingers close, then keep
        # lift/carry moderate enough that the loaded glass remains inside the contact basin.
        approach_phase_rate=0.30,
        transport_phase_rate=0.40,
        waypoint_phases=(0.0, 0.10, 0.20, 0.30, 0.46, 0.66, 1.0),
        grasp_waypoint=3,
        lift_waypoint=4,
        align_waypoint=5,
        ...
        # held tolerance covers contact settling without admitting empty or one-sided closures;
        # the following lift gate still requires the cup itself to rise with the hand.
    ...
    gripper_action = mdp.CurriculumGripperPositionActionCfg(
        ...
        # residual and may relax toward the demonstrated preload across the validated 3 mm
        ...
        neutral_position=0.024,
        open_position=0.04,
        force_open_before_phase_stage=2,
        force_open_before_phase=0.30,
...
class FrankaPourEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # configuration is the far side-approach waypoint of the Newton-IK reference trajectory; the
    # policy must still acquire physical contact, lift, carry, and pour.
    ...
    # The zero-action target sits 3 mm below this ceiling and retained the tall, media-loaded
    # glass through the randomized pour trajectory without the instability caused by higher
    # friction. The policy may relax that squeeze up to this validated preload.
    ...
    # Keep the demonstrated -Y pour direction while moving the receiver throughout the reachable
    # table region. Pairing constrains it to the safe side of each source pose, so the full Y range
    # does not introduce source/receiver overlap.
    ...
    # curriculum never begins perfectly grasp-aligned. Later pre-grasp and grasp waypoints remain
    # aligned, so the reference path teaches recovery instead of removing the reach problem.
    ...
    # IK/reference discontinuity from masquerading as exploration difficulty. The last level
    # contains the complete prevalidated randomization bank.
    curriculum_randomization_extent_levels: tuple[float, ...] = (0.0, 0.05, 0.15, 0.30, 0.50, 0.75, 1.0)
    ...
    def _validate_arm_action_cfg(self) -> None:
        """Validate controller fields used by the task's authored seven-waypoint trajectory."""
        ...
        if arm_action.waypoint_count != 7 or len(waypoint_phases) != 7:
            raise ValueError("Franka Pour requires exactly seven authored arm waypoints and phases.")
        milestone_indices = (
        ...
        if milestone_indices != (1, 3, 4, 5):
            raise ValueError("Franka Pour requires approach, grasp, lift, and align waypoint indices 1, 3, 4, 5.")
        if (

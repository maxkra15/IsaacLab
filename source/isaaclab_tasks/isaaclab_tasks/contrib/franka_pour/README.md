...
Single-worker training applies completed outcomes immediately. Distributed RSL-RL workers queue
outcomes during asynchronous resets, then gather and apply one globally ordered evidence batch at
each rollout boundary. Actor and critic empirical observation normalization is disabled because
task observations are physically scaled at their source.

...
#
# The MuJoCo robot is the authoritative robot model. Its gripper body states
# drive proxy bodies in the VBD cable scene, allowing the cable and plug to
# react to contact without applying VBD contact forces back to the robot.
#
###########################################################################
...
def set_target_pose_kernel(
    ...
    task_plug_body_q_prev: wp.array(dtype=wp.transform),
    # Outputs
    ...
    gripper_target: wp.array(dtype=wp.float32),
):
...
def advance_task_kernel(
    ...
    ee_body_idx: int,
    pos_error_thresholds: wp.array(dtype=wp.vec3),
...
_DEFAULT_ASSET_ROOT = Path(__file__).resolve().parents[5] / "isaaclab_assets" / "data" / "WaterhoseDemo"
ROBOT_PATH = str((_DEFAULT_ASSET_ROOT / "RBY1DF" / "urdf").resolve()) + os.sep
...
def _default_cable_usd_path() -> str:
    return str((_DEFAULT_ASSET_ROOT / "Waterhose" / "Cable008" / "curve" / "cable_SRA_curve03.usda").resolve())
...
class WaterhoseRobotDemoSimulation:
    def __init__(self, viewer, args, preloaded_vbd_scene=None):
        ...
        self.robot_urdf_path = self.asset_root / "RBY1DF" / "urdf" / "robot_edited.urdf"
        self.scene_usd_path = self.asset_root / "Waterhose" / "Cable008" / "Cable008_Body.usda"
        self.cable_usd_path = self.asset_root / "Waterhose" / "Cable008" / "curve" / "cable_SRA_curve03.usda"
        ...
        # MuJoCo drives VBD proxy bodies; VBD contact response stays local to the cable scene.
        self.enable_proxy_sync = True
    ...
    def _init_coupling_buffers(self):
        """Initialize GPU arrays for MuJoCo-to-VBD proxy state sync."""
    ...
    def setup_robot_builder(self):
        ...
        robot_file = str(self.robot_urdf_path)
    ...
    def setup_ik(self):
        ...
        for i, (_name, link_idx) in enumerate(self.ee_configs):
            ...
            target_name = f"ee_{i}"
            self.ik_target_names.append(target_name)
            pose_objectives.append(
        ...
        self.ik_manager = NewtonIKManager(
            NewtonIKManagerCfg(
                command_type="pose",
            ...
            num_envs=1,
            device=str(wp.get_device(getattr(self.args, "device", "cpu"))),
            pose_objectives=pose_objectives,
        )

        self.pos_objs = [self.ik_manager.position_objectives[name] for name in self.ik_target_names]
        self.rot_objs = [self.ik_manager.rotation_objectives[name] for name in self.ik_target_names]
        self.obj_joint_limits = self.ik_manager.joint_limit_objective

        self.ik_joint_q = wp.array(
            self.single_robot_model.joint_q, shape=(1, self.single_robot_model.joint_coord_count)
        )
        self.ik_solver = self.ik_manager.solver
    ...
    def simulate(self):
        """One-way staggered loop: MuJoCo robot drives VBD proxy grippers only."""
    ...
    def _render_vbd_objects(self):
        ...
        # The GL viewer renders a cylinder body plus sphere caps for each capsule,
        ...
        if all_xforms:
            ...
            colors_arr = wp.array(
                [wp.vec3(0.2, 0.6, 0.9)] * len(all_xforms), dtype=wp.vec3, device=self.viewer.device
            )
    ...
    def _print_cable_pose_summary(self, tag: str, cable_results) -> None:
        ...
        for cable_index, result in enumerate(cable_results):
            ...
            print(f"[CABLE_POSE:{tag}] cable={cable_index} last  {fmt_transform(result.cable_body_ids[-1])}", flush=True)

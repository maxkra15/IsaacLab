...
def test_mjwarp_reset_preserves_the_canonical_global_world_entry(monkeypatch):
    """SolverMuJoCo receives Newton's canonical world mask."""
    ...
    monkeypatch.setattr(
        NewtonMJWarpManager,
        "backend",
        SimpleNamespace(model=SimpleNamespace(world_count=2), state_0=object()),
        raising=False,
    )
    ...
    assert reset_masks == [[True, False, False]]


@pytest.mark.parametrize("use_fk_solver", [False, True])
def test_kamino_reset_preserves_the_canonical_global_world_entry(monkeypatch, use_fk_solver):
    """SolverKamino receives Newton's canonical world mask."""
    reset_masks: list[list[bool]] = []
    ...
    monkeypatch.setattr(
        ...
        SimpleNamespace(
            ...
            state_0=SimpleNamespace(joint_q=object(), joint_qd=object()),
        ),
        raising=False,
    )
    monkeypatch.setattr(

...
def test_static_render_meshes_are_imported_as_collision_disabled_world_shapes():
    ...
    dynamic_body = UsdGeom.Xform.Define(stage, "/Root/Dynamic")
    UsdPhysics.RigidBodyAPI.Apply(dynamic_body.GetPrim())
    _define_triangle(stage, "/Root/Dynamic/Visual")

    kinematic_body = UsdGeom.Xform.Define(stage, "/Root/Kinematic")
    UsdPhysics.RigidBodyAPI.Apply(kinematic_body.GetPrim()).CreateKinematicEnabledAttr(True)
    _define_triangle(stage, "/Root/Kinematic/Visual")
    ...
    assert not any(
        label in builder.shape_label
        for label in (
            "/Root/Collision",
            "/Root/Guide",
            "/Root/Invisible",
            "/Root/Dynamic/Visual",
            "/Root/Kinematic/Visual",
        )
    )

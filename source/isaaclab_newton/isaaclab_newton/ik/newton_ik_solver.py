...
class NewtonIKSolver:
    ...
    def __init__(
        ...
        *,
        ...
        self.device = str(self.wp_device)
        ...
        for objective_cfg in pose_objectives:
            target_positions = wp.zeros((num_envs,), dtype=wp.vec3, device=self.wp_device)
        ...
        self.joint_q_out = wp.zeros((num_envs, self.num_coords), dtype=wp.float32, device=self.wp_device)
        self.joint_q_seed_t = torch.zeros((num_envs, self.num_coords), dtype=torch.float32, device=self.device)

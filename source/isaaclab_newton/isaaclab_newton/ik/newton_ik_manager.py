...
class NewtonIKManager:
    ...
    def set_target_pose_from_body_q(
        ...
        *,
        ...
        if body_q_t.ndim == 2:
            ...
            raise ValueError(f"Expected body_q shape (num_bodies, 7) or (num_envs, num_bodies, 7), got {body_q_t.shape}.")
    ...
    def set_joint_seed(self, joint_pos: torch.Tensor, env_ids: Sequence[int] | torch.Tensor | slice | None = None) -> None:
    ...
    def solve(self, joint_pos: torch.Tensor | None = None) -> torch.Tensor:
        ...
        result = wp.to_torch(self.joint_q_out)
        if update_seed:

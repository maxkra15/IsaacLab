...
class ArticulationData(BaseArticulationData):
    ...
    def _ensure_fk_fresh(self) -> None:
        ...
        solver manager's ``forward_pending()``, which calls its solver-specialized FK hook for
        only the articulations dirtied by public asset writes. After a manual joint or root write
        that bypassed the sim step (``write_*_to_sim_*``), ``_fk_timestamp`` is set to ``-1.0``
        to force a refresh on the next read of any property that depends on body poses

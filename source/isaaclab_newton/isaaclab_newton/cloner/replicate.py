...
def _build_newton_builder_from_mapping(
    ...
    for col, _ in enumerate(env_ids.tolist()):
        ...
        if getattr(NewtonManager, "_mpm_object_registry", None):
    ...
    return builder, stage_info, site_index_map, world_xforms, protos
...
class NewtonReplicateContext:
    ...
    def __init__(
        ...
        *,
        ...
        active_cfg = PhysicsManager._cfg
        self.simplify_meshes = (
            bool(getattr(active_cfg, "simplify_meshes", True)) if simplify_meshes is None else simplify_meshes
        )
    ...
    def replicate(self) -> tuple[ModelBuilder, object, dict]:
        ...
        builder, stage_info, site_index_map, world_xforms, protos = _build_newton_builder_from_mapping(
            stage=self.stage,
            sources=sources,
            env_ids=env_ids,
            mapping=mapping,

...
class CoupledSolverEntryCfg:
    ...
    """Whether coupling uses the solver's specialized effective-mass hook.

    Set to ``False`` to use the generic body/particle mass and inertia stored
    on the entry's model view. This is useful for virtual contact proxies that
    should reproduce the inertial properties of the selected links rather than
    the articulated-system effective mass seen through those links.
    """
...
class CoupledProxyCfg:
    ...
    collision_pipeline_factory: Callable[[ModelView], object | None] | None = None
...
class CoupledAdmmContactPairCfg:
    """Configuration for one ADMM contact interface between named entries."""

    source: str = MISSING
    """Name of one solver entry in the contact pair."""

    destination: str = MISSING
    """Name of the other solver entry in the contact pair."""
...
class CoupledSolverCfg(NewtonSolverCfg):
    ...
    """Whether Isaac Lab runs a shared outer Newton collision pipeline.

    ``None`` uses the algorithm default: disabled for proxy coupling, whose
    destination mappings own collision pipelines, and enabled for ADMM.
    When disabled, entries that require external contacts and are not proxy
    destinations receive an entry-local pipeline.
    """
...
class CoupledAdmmSolverCfg(CoupledSolverCfg):
    ...
    contact_pairs: list[CoupledAdmmContactPairCfg] = field(default_factory=list)
    """Explicit contact interfaces between named solver entries."""

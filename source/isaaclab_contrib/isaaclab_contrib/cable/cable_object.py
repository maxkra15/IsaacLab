...
def _active_solver_cfg_supports_cables() -> bool:
    ...
    from isaaclab_contrib.coupling import CoupledSolverCfg
...
class CableObject(Articulation):
    ...
    def _register_cable(self) -> CableRegistryEntry:
        ...
        cable_hooks_installed = add_registered_cables_to_builder in getattr(
            SimulationManager, "_per_world_builder_hooks", ()
        )
        if _active_solver_cfg_supports_cables() and (
            not hasattr(SimulationManager, "_cable_registry") or not cable_hooks_installed
        ):
